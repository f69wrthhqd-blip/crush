...
func (r *RenameToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	if opts.IsPending() {
		return pendingTool(sty, i18n.T("chat.rename_symbol"), opts.Anim, opts.Compact)
	}
	...
	header := toolHeader(sty, opts.Status, i18n.T("chat.rename_symbol"), cappedWidth, opts, toolParams...)
	if opts.Compact {
		return header
	}
